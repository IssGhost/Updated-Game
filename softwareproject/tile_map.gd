...
func place_bed_in_random_room():
	if root_leaf.rooms.size() == 0:
		print("No rooms available to place the bed.")
		return
	var random_room = root_leaf.rooms[randi_range(0, root_leaf.rooms.size() - 1)]
	var room_center = random_room.rect.position + random_room.rect.size / 2
	var bed_instance = bed_boss_scene.instantiate()
	bed_instance.position = room_center * 32  # Assuming tile size is 32x32

	add_child(bed_instance)

	print("Bed placed in room ID:", random_room.id, "at position:", bed_instance.position)
